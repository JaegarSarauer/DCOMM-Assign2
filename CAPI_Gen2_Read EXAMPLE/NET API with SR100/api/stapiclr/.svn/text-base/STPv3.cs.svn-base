...
namespace SkyeTek
{
    ...
    namespace STPv3
    {
        ...
        public static class STPv3Commands
        {
            ...
            public static STPv3Command SCAN_EAS = new STPv3Command(0x0503, "Scan EAS", 300, true, false, false);
        ...
        public enum Commands: ushort
        {
            ...
            WRITE_TAG = 0x0103,
            ACTIVATE_TAG = 0x0104,
            DEACTIVATE_TAG = 0x0105,
            SET_TAG_BIT_RATE = 0x0106, 
            GET_TAG_INFO = 0x0107,
            GET_LOCK_STATUS = 0x0108, 
            KILL_TAG = 0x0109,
            REVIVE_TAG = 0x010A,
            ERASE_TAG = 0x010B, 
            FORMAT_TAG = 0x010C,
            DESELECT_TAG = 0x010D, 
            READ_TAG_CONFIG = 0x0110,  
            WRITE_TAG_CONFIG = 0x0111, 
            AUTHENTICATE_TAG = 0x0201,
            SEND_TAG_PASSWORD = 0x0202, 
            INIT_SECURE_MEMORY = 0x0203,
            SETUP_SECURE_MEMORY = 0x0204, 
            GET_APPLICATION_IDS = 0x0301, 
            SELECT_APPLICATION = 0x0302, 
            CREATE_APPLICATION = 0x0303, 
            DELETE_APPLICATION = 0x0304,
            GET_FILE_IDS = 0x0401, 
            SELECT_FILE = 0x0402, 
            CREATE_FILE = 0x0403, 
            GET_FILE_SETTINGS = 0x0404, 
            CHANGE_FILE_SETTINGS = 0x0405, 
            READ_FILE = 0x0406, 
            WRITE_FILE = 0x0407, 
            DELETE_FILE = 0x0408, 
            CLEAR_FILE = 0x0409,
            CREDIT_VALUE_FILE = 0x040A, 
            DEBIT_VALUE_FILE = 0x040B, 
            LIMITED_CREDIT_VALUE_FILE = 0x040C, 
            GET_VALUE = 0x040D, 
            COMMIT_TRANSACTION = 0x040E,
            ABORT_TRANSACTION = 0x040F, 
            READ_RECORDS = 0x0410, 
            WRITE_RECORD = 0x0411, 
            CHANGE_KEY_SETTINGS = 0x0412, 
            GET_KEY_SETTINGS = 0x0413, 
            GET_KEY_VERSION = 0x0414,
            CHANGE_KEY = 0x0415, 
            ENABLE_EAS = 0x0501, 
            DISABLE_EAS = 0x0502, 
            SCAN_EAS = 0x0503, 
            ...
            STORE_KEY = 0x0601,
            LOAD_KEY = 0x0602,
            INTERFACE_SEND = 0x0701,
            TRANSPORT_SEND = 0x0702,
            INITIATE_PAYMENT = 0x0801,
            COMPUTE_PAYMENT = 0x0802,
            RESET_DEVICE = 0x1102,
            ENTER_BOOTLOAD = 0x1103,
            READ_SYSTEM_PARAMETER = 0x1201,
            WRITE_SYSTEM_PARAMETER = 0x1202,
            STORE_DEFAULT_SYSTEM_PARAMETER = 0x1301,
            RETRIEVE_DEFAULT_SYSTEM_PARAMETER = 0x1302,
            AUTHENTICATE_READER = 0x1401,
            ENABLE_DEBUG = 0x1402,
            DISABLE_DEBUG = 0x1403,
            DEBUG_MESSAGES = 0x1404,
            ENTER_PAYMENT_SCAN_MODE = 0x1405
        }
        ...
        public enum STPv3ResponseCode : ushort
        {
            ...
            /*READER_TIME_OUT is not a response code from The reader,
             * we use it , to handle InventoryTag, which can time out and it is normal, 
             * since it is a acynch communication
             */
            READER_TIME_OUT= 0x81FF
        }
            ...
            public TagType TagType
            {
                get
                {
                         ...
                         (this.m_request.RID[3] != 0xFF))
                    {
                        ...
                        && (((ushort)this.m_request.Tag.Type & 0x000f) == 0))
                    {
                        ushort offset = 5;
                        return (TagType)((this.m_responseBuffer[offset] << 8) | (this.m_responseBuffer[offset + 1]));
                    }
        ...
        public class STPv3Request
        {
            ...
            private bool isValidSTPv3Response(byte[] response)
            {

                ushort length = 0;
                ushort crc16 = 0;

                if (response.Length < 7)       //too short response
                    return false;
                if (response[0] != STPv3.STX)  //invalid start of frame
                    return false;
                length = (ushort)((ushort)(response[1] << 8) | (response[2]));
                if (length > STPv3.MAX_LENGTH) // too long message
                    return false;
                if (length != response.Length - 3)  //message length different that what the reader says 
                    return false;
                crc16 = (ushort)((response[response.Length - 2] << 8) | response[response.Length - 1]);
                if (crc16 != STP.Utils.crc16(0, response, 1, length))  //wrong CRC 
                    return false;
            ...
            public STPv3Response GetResponse()
            {
                ...
                byte[] T_out = new byte[7] {0x02, 0x00, 0x04, 0x81, 0xff, 0xf9, 0x0d };   //time out buffer
                ...
                if (this.m_mode == STP.ProtocolMode.BINARY)
                {
                    ...
                    /* The stream Socket works a bit different than
                     * Serial port stream. Since it is a network, delay can 
                     * be substantial from network equipment.Skyetek messages may come 
                     * fragmented( most likely) so on. Besides if there is no data on the stream
                     * it throws exception and closes socket. we need to handle that gracefully
                     */
                     ...
                     * 
                     */ 
                    if (m_device.Type == DeviceType.TCP)
                    {
                        if (m_device.ReadTimeout == -1)
                            timeout = -1;
                        else
                            timeout = 1000*(m_device.ReadTimeout); //in us

                        try
                        {
                            if (this.m_device.CanRead)
                            {
                                byte[] b = new byte[2048];
                               
                                if (m_device.TcpPollData(timeout))
                                {
                                    if (m_device.AvailableTCPDataBytes == 0) //socket closed
                                           throw new SocketException(); 

                                    int rbyte = m_device.ReadByte();
                                    if (rbyte != STPv3.STX)
                                        throw new SocketException();
                                    byte[] len = new byte[2];
                                    chunk = 0;
                                    ...
                                    do
                                    {
                                        chunk = m_device.Read(len, offset, remaining);
                                        remaining -= chunk;
                                        offset += chunk;

                                    } while ((m_device.isDataAvailable) && (remaining != 0));

                                    if (remaining!=0) throw new SocketException();//not enough data and stream is depleted
                                        
                                    length = (ushort)((ushort)(len[0] << 8) | (len[1]));
                                    if (length > STPv3.MAX_LENGTH) // too long message
                                        throw new SocketException();
                                    ...
                                    do
                                        {
                                            chunk = m_device.Read(b, offset, remaining);
                                            remaining -= chunk;
                                            offset += chunk;
                                    ...
                                    b[2] = (byte)(length & 0xff);
                                     ...
                                     byte[] Stpv3Resp = new byte[offset];
                                     Array.Copy(b, Stpv3Resp, offset);    //shrink the message to length
                                     if (this.isValidSTPv3Response(Stpv3Resp)) return new STPv3Response(this, Stpv3Resp);
                                     else //response is not valid, return time out
                                        {
                                            Debug.Print("Invalid Response STPv3 Format: Exception thrown");
                                            //return new STPv3Response(this, T_out); //time out
                                            Debug.Print(BitConverter.ToString(Stpv3Resp).Replace("-", ""));
                                            throw new SocketException();
                                        }
                        ...
                        {
                            //when NetworkStream does not succeed reading from Stream, it throws exception
                            //and closes socket, so reopen it.
                        ...
                        catch (ArgumentException ae) //thrown by CRC16
                        {
                    ...
                    if (m_device.Type == DeviceType.SERIAL)
                    {
                        try
                        {
                            if (m_device.CanRead)
                            {
                                byte[] b = new byte[2048];
                                int rbyte = m_device.ReadByte();
                                if(rbyte==-1)throw new TimeoutException();
                                if (rbyte != STPv3.STX)
                                    throw new Exception("did not Recognized STX");
                                byte[] len = new byte[2];
                                chunk = 0;
                                offset = 0;
                                remaining = len.Length;

                                while ((chunk = m_device.Read(len, offset, remaining)) > 0) 
                                {
                                    remaining -= chunk;
                                    offset += chunk;
                                }

                                if (remaining != 0) throw new Exception("Not enough data to get message length operation");
                                length = (ushort)((ushort)(len[0] << 8) | (len[1]));
                                if (length > STPv3.MAX_LENGTH) // too long message
                                    throw new Exception("Message Length Too Long");
                                chunk = 0;
                                offset = 3;
                                remaining = length;

                                while ((chunk = m_device.Read(b, offset, remaining)) > 0)
                                {
                                    remaining -= chunk;
                                    offset += chunk;
                                }
                                ...
                                else //response is not valid, return time out
                                {
                                    ...
                                    //return new STPv3Response(this, T_out); //time out
                                    Debug.Print(BitConverter.ToString(Stpv3Resp).Replace("-", ""));
                                    throw new Exception("Invalid STPv3 Format");
                                }
                        ...
                        {
                            Debug.Print("Serial Port Time Out");
                            return new STPv3Response(this, T_out); //time out
                        }
                    ...
                    try { read = this.m_device.Read(lbuf, 0, 2); }
                    catch (Exception ex) { }
